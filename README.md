...
<br> Support or issues reported by users who choose to use the mod outside the base modpack will not be provided.
...
- **Creative items:**
  ...
  - Teleport to Scenery (Teleport players to the Spectator Spawn Point upon use) **OP ONLY**
  - Create RTP Slot (Creates a RTP slot upon use) **OP ONLY**
  - Trigger RTP (Trigger the RTP function and randomly teleports players across all the slots upon use) **OP ONLY**
  - Add/Remove Fake Players (Spawns/Kill a fake player for testing, requires [Carpet](https://modrinth.com/mod/carpet) mod) **OP ONLY**
- **More Panel block variants**
- **More Moquette color variants**
- **More Steel Ornament color variants**
- **Ish Plush**
  - Meow :3
...
- **Cleaner Ability Player Limit**: The Deep Cleaning ability from Kin's Wathe **Cleaner** role will automatically be disabled when the number of alive survival players falls below a configurable threshold, preventing it from being overpowered in the late game when there are very few players left, ultimately soft-locking the remaining players to have a chance to win.
...
- Admins can clear other player's pronouns if needed using the `/watheextended:pronouns clear <target>` command.
...
- **Defense Shield kill bypass fix**: The `AllowPlayerDeath` event is now properly fired before executing a kill, fixing cases where Defense Shields were bypassed (e.g. pill, defense vial, dream imprint), this also fixes issues regarding role convertion that requires a specific player to be killed happening when the said player have a Defense Shield when receiving damage.
...
- **Duplicate gun pickup bug fix**: Players could previously pick up extra guns by picking a gun on the ground while already holding one in their cursor.
- **Mood component NBT persistence fix**: `nextTaskTimer` and `timesGotten` task data are now properly serialized/deserialized in NBT, preventing mood task progress from being lost when reconnecting during an active game.
...
- **Wathe & Iris conflict**: Fixed a crash caused by a conflict between Wathe and Iris Shaders when certain specific shaders are used.
- 
### Noelle's Roles
...
- **Feather Modifier Fix**: Kin's Wathe conflicts with Noelle's Roles Feather modifier, Wathe: Extended reapplies the `SLOW_FALLING` effect with an infinite duration for players having this modifier so it never expires.

### Stupid Express
- **Lovers crash fix**: Catches a `NullPointerException` that would crash the game in the Lovers HUD renderer during spectator mode, ultimately causing the client to crash if a lover died while the client was looking at them.
...
- @doctor4t and the Lady Snake team for making the original idea of The Last Voyage of the Harpy Express and WATHE
