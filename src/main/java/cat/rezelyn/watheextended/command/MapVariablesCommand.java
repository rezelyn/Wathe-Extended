...
public class MapVariablesCommand {
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(CommandManager.literal("watheextended:mapVariables").requires(source -> source.hasPermissionLevel(2)).then(CommandManager.literal("set").then(CommandManager.literal("readyAreaSpawnPosition").then(CommandManager.argument("location", Vec3ArgumentType.vec3()).then(CommandManager.argument("rotation", RotationArgumentType.rotation()).executes(context -> setReadyAreaSpawnPosition(context, Vec3ArgumentType.getPosArgument(context, "location"), RotationArgumentType.getRotation(context, "rotation")))))).then(CommandManager.literal("lobbyArea").then(CommandManager.argument("x1", DoubleArgumentType.doubleArg()).then(CommandManager.argument("y1", DoubleArgumentType.doubleArg()).then(CommandManager.argument("z1", DoubleArgumentType.doubleArg()).then(CommandManager.argument("x2", DoubleArgumentType.doubleArg()).then(CommandManager.argument("y2", DoubleArgumentType.doubleArg()).then(CommandManager.argument("z2", DoubleArgumentType.doubleArg()).executes(MapVariablesCommand::setLobbyArea))))))))));
    }
    ...
    private static int setReadyAreaSpawnPosition(CommandContext<ServerCommandSource> context, PosArgument location, PosArgument rotation) {
        ...
        Vec2f yaw = rotation.toAbsoluteRotation(source);

        MapVariablesWorldComponent.PosWithOrientation posWithOrientation = new MapVariablesWorldComponent.PosWithOrientation(pos, yaw.y, yaw.x);

        WatheExtendedWorldComponent.KEY.get(source.getWorld()).setReadyAreaSpawnPos(posWithOrientation);

        source.sendMessage(Text.translatable("wathe.map_variables.set", "readyAreaSpawnPosition", String.format("%.4f %.4f %.4f (yaw=%.4f, pitch=%.4f)", pos.x, pos.y, pos.z, (double) yaw.y, (double) yaw.x)));
        return 1;
    }
